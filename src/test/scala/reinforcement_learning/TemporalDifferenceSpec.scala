...
class TemporalDifferenceSpec extends FlatSpec with Matchers {
  ...
  it should "execute SARSA on blackjack" in {
    val environment = BlackJackEnvironment()
    val policy = TemporalDifference.SARSA(environment, 0.001, 0.6, 0.001, 10)
  ...
  it should "execute SARSA on random walk" in {
    val environment = RandomWalkEnvironment()
    //    val policy = TemporalDifference.SARSA(environment, 0.5, 1.0, 0.1, 50)
    //    println(policy)

  }
   it should "execute SARSA on cliff walk" in {
    val rows = 4
    val cols = 12
    val environment = new CliffWalkEnvironment(cols, rows)
    val policy = TemporalDifference.SARSA(environment, 0.1, 1.0, 0.1, 100000)

    val str = (0 until rows).map { r => (0 until cols).map(c => r * cols + c).map(policy.getOrElse(_, "")).mkString("\t") }.mkString("\n")
    println(str)
  }
  it should "execute Q-learning on cliff walk" in {
    val rows = 4
    val cols = 12
    val environment = new CliffWalkEnvironment(cols, rows)
    val policy = TemporalDifference.QLearning(environment, 0.2, 1.0, 0.1, 100000)
  ...
  it should "execute Q-learning on blackjack" in {
    ...
    val policy = TemporalDifference.QLearning(environment, 0.25, 1.0, 0.2, 50)
